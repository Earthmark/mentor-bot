...
{
    [FromQuery] public string? UserId { get; init; }

    [FromQuery] public string? Lang { get; init; }

    [FromQuery] public string? Desc { get; init; }

    [FromQuery] public string? Session { get; init; }

    [FromQuery] public string? SessionId { get; init; }

    [FromQuery] public string? SessionUrl { get; init; }

    [FromQuery] public string? SessionWebUrl { get; init; }
}
...
public record Ticket
{
    ...
    public IEnumerable<(string Name, string Value, bool Inline)> Fields()
    {
        IEnumerable<(string Name, string Value, bool Inline)?> NullableFields()
        {
            ...
            yield return Field("Mentor Neos ID", Mentor?.ResoUserId, true);
    ...
    public MentorTicketDto ToMentorDto()
    {
        return new MentorTicketDto(this);
    }
}
...
public record MentorTicketDto : TicketDto
{
    ...
    public string? MentorId => _ticket.Mentor?.ResoUserId;
