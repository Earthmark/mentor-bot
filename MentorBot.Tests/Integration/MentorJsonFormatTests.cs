...
public class MentorJsonFormatTests(SignalWebApplicationFactory factory) : IClassFixture<SignalWebApplicationFactory>
{
    private readonly WebSocketClient _wsClient = factory.Server.CreateWebSocketClient();
    ...
    public async Task CreateClaimAndCancelTicketSequence()
    {
        ...
        Assert.True(DateTime.TryParse(Assert.Contains("created", mentorMsg), out _));
        ...
        using (var mentee = await _wsClient.BindUser($"/ws/mentee/{ticket}"))
        {
            ...
            Assert.True(DateTime.TryParse(Assert.Contains("created", mentorMsg2), out _));
